...
from backend.app.infrastructure.adapters.repository import (
    SQLAlchemyFIARRepositoryAdapter,
)
...
class TestSQLAlchemyFIARRepositoryAdapter:
    ...
    async def test_get_ticker_with_technical_and_historical_data(
        self, formatted_fi_test_data, mock_dependencies
    ):
        """
        ...
        mock_session.execute = AsyncMock(
            side_effect=[ticker_result, technical_result, historical_result]
        )
        repo = SQLAlchemyFIARRepositoryAdapter(mock_session)
        ...
        mock_session.execute.side_effect = SQLAlchemyError(
            "Error retrieving ticker with technical and historical data."
        )
        with pytest.raises(
            SQLAlchemyError,
            match="Error retrieving ticker with technical and historical data.",
        ):
            await repo.get_ticker_with_technical_and_historical_data(code="ABC")
    ...
    async def test_add_historical_data_bulk(
        self, formatted_fi_test_data, mock_dependencies
    ):
        """
        ...
        mock_session.add_all.side_effect = SQLAlchemyError(
            "Error adding historical data in bulk."
        )
        with pytest.raises(
            SQLAlchemyError, match="Error adding historical data in bulk."
        ):
            await repo.add_historical_data_bulk(
                historical_data_list=mock_historical_data
            )
