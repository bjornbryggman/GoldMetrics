...
from backend.app.infrastructure.adapters.event_bus import (
    RabbitMQEventBusAdapter,
    RedisEventStoreAdapter,
)
...
class Container(containers.DeclarativeContainer):
    ...
    wiring_config = containers.WiringConfiguration(
        modules=["backend.app.presentation.endpoints"]
    )
    ...
    database_engine = providers.Singleton(
        create_async_engine, config.provided.async_timescaledb_url
    )
    session_factory = providers.Factory(
        async_sessionmaker,
        database_engine.provided,
        expire_on_commit=False,
        class_=AsyncSession,
    )  # type: async_sessionmaker[AsyncSession]
    unit_of_work = providers.Factory(
        SQLAlchemyUnitOfWorkAdapter, session_factory.provided
    )
    ...
    event_store = providers.Singleton(
        RedisEventStoreAdapter, redis_url=config.provided.redis_url
    )
    event_bus = providers.Singleton(
        RabbitMQEventBusAdapter,
        rabbitmq_url=config.provided.rabbitmq_url,
        event_store=event_store.provided,
    )
    telegram_notification = providers.Singleton(
        TelegramNotificationAdapter,
        config.provided.telegram_bot_token,
        config.provided.telegram_chat_id,
    )
    ...
    eodhd_api_client = providers.Factory(
        EODHDAPIClient, config.provided.eodhd_api_key, rate_limit=1000
    )
